...
{
  [Serializable]
  public class KF_FloatingBrainMutation : BaseMutation
  {
    public KF_FloatingBrainMutation() => this.DisplayName = "{{psionic|Pituitary Gland}}";
    ...
    public override string GetDescription() => "Your {{psionic|pituitary gland}} {{phase-harmonic|thrums with Ptoh-song}}.\n\nWith time who knows how {{psionic|your mind will grow}}.";
    ...
    public override bool FireEvent(Event E)
    {
      if (E.ID == "AfterLevelGainedEarly")
      {
			...
			&& this.ParentObject.GetGenotype() == "KF_Fleshbeast")
		{
			...
			this.ParentObject.GetStat("DV").BaseValue += 1;
			int _level = this.ParentObject.Stat("Level");
			if( _level == 2)
			{
				this.RapidAdvancementHelper(9);
				StatusScreen.BuyRandomMutation(this.ParentObject);
				StatusScreen.BuyRandomMutation(this.ParentObject);
				this.ParentObject.GetStat("AV").BaseValue += 2;
				this.ParentObject.GetStat("DV").BaseValue += 2;
			}
			if ( _level % 3 == 0)
			{
				this.ParentObject.GetStat("AV").BaseValue += 1;
			}			
			if( _level % 4 == 0)
			{
				StatusScreen.BuyRandomMutation(this.ParentObject);
